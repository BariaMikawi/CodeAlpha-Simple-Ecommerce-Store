...
def home_view(request):
    ...
    for product in products:
        if not product.product_image:
            product.product_image = 'products/default.jpg'  # Fallback to default image
    offers_category = Category.objects.filter(name="Offers").first()
    offers_products = Product.objects.filter(category=offers_category) if offers_category else []

    # Check for category filtering
    category_name = request.GET.get('category')  # Category name is passed as a query parameter
    if category_name:
        # Filter products by category name (case-insensitive)
      products = products.filter(category__name__iexact=category_name)


    # Get all categories to display in the template
...
def search_view(request):
    ...
    return render(request, 'ecom/index.html', {'search_results': search_results})
...
def admin_add_product_view(request):
    ...
    if request.method == 'POST':
        ...
        if productForm.is_valid():
            productForm.save()
            return HttpResponseRedirect('admin-products')  # Redirect after saving
    categories = Category.objects.all()  # Fetch all categories for the form
    return render(request, 'ecom/admin_add_products.html', {'productForm': productForm, 'categories': categories})
...
def customer_home_view(request):
    ...
    # Fetch the "Offers" category
    offers_category = models.Category.objects.filter(name="Offers").first()
    offers_products = models.Product.objects.filter(category=offers_category) if offers_category else []
...
def product_search(request):
    ...
    if min_price:
        products = products.filter(price__gte=min_price)  # Filter by min price
    
    if max_price:
        products = products.filter(price__lte=max_price)  # Filter by max price
